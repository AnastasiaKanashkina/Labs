...
public class Crawler {
	...
	// —писок всех просмотренных сайтов .
	static LinkedList<URLDepthPair> allSitesSeen = 
			new LinkedList<URLDepthPair>();
	// —писок всех непросмотренных сайтов
	static LinkedList<URLDepthPair> sitesToVisit = new LinkedList<URLDepthPair>();
	
	/**
	 * ћетод сканирует интернет, начина€ с указанного сайта и на указанную глубину
	 * @param startURL - начальный URL
	 * @param maxDepth - максимальна€ глубина поиска
	...
	public static void crawl(String startURL, int maxDepth)
		...
		//ѕродолжать до тех пор пока не будут просканированы все страницы
		...
		while (!sitesToVisit.isEmpty()) {
			...
			try {
				...
				System.out.println("ќсуществл€етс€ соединение с " + currPair.getURLString());
				PrintWriter output =
				        new PrintWriter(socket.getOutputStream(), true);
			    BufferedReader input =
			        new BufferedReader(
			            new InputStreamReader(socket.getInputStream()));
			    //ќтправка HTTP запроса
			 ...
			 // Ќахождение ссылок на странице
			    String line;
			    int lineLength;
			    int shiftIdx;
			    while ((line = input.readLine()) != null) {
			    	// ѕроверка есть ли у строки ссылка
			    ...
			    // ѕометка страницы как просмотренной и закрытие сокета.
			    socket.close();
	...
	/**
	 * Ётот метод сканирует интернет, начина€ с указанной в args[0] на максимальную глубину
	 * arg[1]
	 **/
